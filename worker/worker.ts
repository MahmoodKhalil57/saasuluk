...
import {
  ...
  cancelPendingOrder,
  reapAbandonedOrders,
  sweepAbandonedCartEmails,
  refundOrder,
  sendOrderReceipt,
  crudAfterUpdate,
  CRUD_AFTER_UPDATE_TABLES,
...
const document = hardenDocument(
  annotateSource(annotateAccess(mergeAuth(annotateCosts(built.backend.document, costs), {}, { securitySchemes }))),
); // cost + access + source (provenance) facets + baseline hardening
...
const app = new Hono<{
  Bindings: Env;
  Variables: { tokenUser?: string; sessionUser?: string; sessionEmail?: string; isAdmin?: boolean };
}>();
...
app.on(["GET", "POST"], "/api/auth/*", async (c) => {
  try {
    ...
    return c.json({ error: "auth unavailable", detail: String((e as Error)?.message ?? e) }, 503);
  }
});
...
app.use("*", async (c, next) => {
  if (!configChecked) {
    configChecked = true;
    const { problem } = loadConfig(c.env as unknown as Record<string, string | undefined>);
  ...
  if (h?.startsWith("Bearer sk_")) {
    const u = await verifyApiToken(drizzle(c.env.DB), h);
    ...
    try {
      ...
      } // email stashed so checkout snapshots it onto the order + sends a receipt
    } catch {
      /* anonymous */
    }
  }
...
app.use("*", async (c, next) => {
  if (!RL_KV && (c.env as Env).RATE_LIMIT_KV) RL_KV = (c.env as Env).RATE_LIMIT_KV;
  return next();
});
app.use(
  "*",
...
app.use("*", async (c, next) => {
  ...
  await c.env.DB.prepare("INSERT INTO cost_event (at, principal, operation, action, total_micro_usd, breakdown) VALUES (?,?,?,?,?,?)")
    .bind(Date.now(), who, op, c.req.header("x-suluk-action") ?? null, totalMicroUsd, JSON.stringify(breakdown))
    .run();
});
...
const routes: RouteContract[] = built.backend.routes.map((r) => {
  ...
  if (!h) {
    h = d1Crud(def.table, def.ownerCol, def.access);
...
const refProjected = (c: Context) => {
  ...
  const b = refBase(c);
  return v ? projectDocument(b, v, CANON_HASH) : b;
};
app.get("/reference", (c) =>
  scalarV4Response(refBase(c as unknown as Context), {
    cdn: SCALAR_FORK,
...
app.get("/reference/insights", (c) =>
  referenceInsightsResponse(refProjected(c as unknown as Context), { costLedgerUrl: "/cost", whoamiUrl: "/api/whoami" }),
);
...
app.get(
  ...
  (c) =>
    new Response(generateSdk(document, { baseURL: new URL(c.req.url).origin }), {
      headers: { "content-type": "application/typescript; charset=utf-8", "content-disposition": 'attachment; filename="saasuluk-sdk.ts"' },
    }),
); // a complete typed ofetch SDK from the contract
app.get(
  "/conformance.test.ts",
  (c) =>
...
app.get("/openapi.json", (c) => {
  // canonical (full, auth-free); ?as= → a provable-subset PROJECTION
...
app.get("/cost", async (c) => {
  ...
  const stmt = admin
    ...
    : c.env.DB.prepare(
        "SELECT at, principal, operation, action, total_micro_usd, breakdown FROM cost_event WHERE principal = ? ORDER BY at DESC LIMIT 2000",
      ).bind(who ?? " ");
  const { results } = await stmt.all();
  const events: CostEvent[] = (results as Record<string, unknown>[]).map((r) => ({
    at: Number(r.at),
    principal: (r.principal as string) ?? undefined,
    operation: r.operation as string,
    action: (r.action as string) ?? undefined,
    totalMicroUsd: Number(r.total_micro_usd),
...
const isSignedIn = (c: Context): boolean => {
  ...
  return !!(g.get("sessionUser") || g.get("tokenUser"));
};
...
app.route(
  ...
  mcpApp({
    ...
    instructions:
      "Browse the saasuluk store: list and read products, posts, and categories. The tool set mirrors the caller's role — anonymous callers see the public catalog.",
  }),
);
...
app.route(
  ...
  chatApp({
    ...
    title: "saasuluk",
    greeting:
      "Hi! I'm the saasuluk assistant. I can find products, compare plans, add things to your cart (no sign-in needed), switch the theme, navigate the site, and dig through the docs. What can I do for you?",
    system:
...
app.get("/api/health", (c) =>
  c.json({ ok: true, on: "cloudflare-workers", name: "saasuluk", build: BUILD_ID }, 200, { [DEPLOYMENT_HEADER]: BUILD_ID }),
); // build id → @suluk/seo skew-protection
app.get("/og.svg", (c) =>
  c.body(
    ogImageSvg({
...
); // dynamic branded OG card (@suluk/seo)
...
app.post("/api/stripe/webhook", async (c) => {
  ...
  await webhookRouter()
    ...
    .on(STRIPE_EVENTS.checkoutCompleted, async () => {
      if (oid && (await markOrderPaid(c as unknown as Parameters<typeof markOrderPaid>[0], dz, oid)))
        await sendOrderReceipt(c as unknown as Parameters<typeof sendOrderReceipt>[0], dz, oid);
    })
    .on(STRIPE_EVENTS.checkoutExpired, async () => {
      if (oid) await cancelPendingOrder(dz, oid);
    }) // buyer abandoned the hosted checkout → release the pending order
    // charge.refunded also fires on PARTIAL refunds (refunded=false); only a FULL refund cancels + restocks the order.
    .on(STRIPE_EVENTS.chargeRefunded, async () => {
      if (oid && obj?.refunded === true) await refundOrder(dz, oid);
    })
    ...
    .on(STRIPE_EVENTS.disputeClosed, async () => {
      ...
      if (did) await refundOrder(dz, did);
      else disputeUnresolved = true; // can't resolve → 5xx below so Stripe re-delivers
    })
    .handle(evt);
...
app.notFound(async (c) => {
  if (c.req.method === "GET" && (c.req.header("accept") ?? "").includes("text/html")) {
    const res = await (c.env as Env & { ASSETS: { fetch: (req: Request) => Promise<Response> } }).ASSETS.fetch(
...
export default {
  ...
  async scheduled(_event: unknown, env: Env, ctx: { waitUntil: (p: Promise<unknown>) => void }) {
    ...
    ctx.waitUntil(
      sweepAbandonedCartEmails(drizzle(env.DB), {
        apiKey: env.RESEND_API_KEY,
        from: env.EMAIL_FROM,
        origin: env.BASE_URL ?? "https://saasuluk.saastemly.com",
      }).catch(() => 0),
    );
    if (!env.STRIPE_SECRET_KEY) return;
    ctx.waitUntil(
      sweepBillingUsage(drizzle(env.DB), env.STRIPE_SECRET_KEY, env.STRIPE_METER_EVENT_NAME ?? METER_EVENT_DEFAULT).catch(() => ({
        swept: 0,
        reported: 0,
      })),
    );
