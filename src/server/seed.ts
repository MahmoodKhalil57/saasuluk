/**
 * The meta seed — the store sells the code that renders it. Products are the real tiers + modules of this very
 * starter; the blog, FAQs, reviews and discount codes make the platform feel alive. ONE source: the dev DB runs
 * this on boot (`db.ts`), and `scripts/seed.sql` (generated from it) is applied to the remote D1 on deploy.
 * Idempotent — fixed ids + INSERT OR REPLACE, so re-running never duplicates.
 */
