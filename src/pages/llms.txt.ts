...
export const GET: APIRoute = ({ site }) => {
  ...
  const body = llmsTxt({
    title: "saasuluk",
    summary:
      "A premium ecommerce + SaaS starter where every layer — data, API, docs, typed client, UI, admin, cost — is projected from ONE OpenAPI v4 contract. Built on Suluk (Astro + Hono + Drizzle on Cloudflare).",
    details:
      "saasuluk sells the very code that renders it — the products below are real slices of this repository. The whole API is live and typed; the cost of every request is metered to a public ledger.",
    sections: [
      {
        title: "Products",
        links: SEED_PRODUCTS.filter((p) => p.status === "published").map((p) => ({
          title: p.name,
        ...
        links: [
          {
            ...
            url: `${base}/mcp`,
            description:
              "Model Context Protocol endpoint (Streamable-HTTP, POST JSON-RPC). The same OpenAPI v4 contract projected into read-only tools — list/get products, posts, and categories. Call `initialize` then `tools/list`.",
          },
        ],
      },
      {
