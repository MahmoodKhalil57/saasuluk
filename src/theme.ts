...
export function altSchemesCss(): string {
  return SCHEMES.map((s) => projectScheme(s.name, s.css))
    .filter(Boolean)
    .join("\n");
}
...
export const SCHEME_SWATCHES: SchemeSwatch[] = [
  ...
  ...SCHEMES.map((s) => {
    const sw = parseSwatch(s.name, s.css);
    return { name: s.name, label: prettyLabel(s.name), light: sw?.light ?? [], dark: sw?.dark ?? [] };
  }),
];
